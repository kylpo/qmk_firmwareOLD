...
enum combos { C_X, C_Z, C_J, C_Q };

#define M_CMD_CLICK LGUI(KC_MS_BTN1)

const uint16_t PROGMEM combo_x[] = {KC_A, KC_E, COMBO_END};
const uint16_t PROGMEM combo_z[] = {KC_E, KC_I, COMBO_END};
const uint16_t PROGMEM combo_j[] = {KC_T, KC_SPACE, COMBO_END};
const uint16_t PROGMEM combo_q[] = {KC_O, KC_T, COMBO_END};

combo_t key_combos[COMBO_COUNT] = {[C_X] = COMBO(combo_x, KC_X), [C_Z] = COMBO(combo_z, KC_Z), [C_J] = COMBO(combo_j, KC_J), [C_Q] = COMBO(combo_q, KC_Q)};
